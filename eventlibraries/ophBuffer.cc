...
// the constructor first ----------------
ophBuffer::ophBuffer (const char *filename, PHDWORD * where, const int length, int &status
...
ophBuffer::ophBuffer (int fdin, PHDWORD * where, const int length
		, const int irun, const int iseq)
{
  fd = fdin;
  our_fd = 0;
...
// ---------------------------------------------------------
int ophBuffer::nextEvent( const unsigned int evtsize, const int etype, const int evtseq)
{
