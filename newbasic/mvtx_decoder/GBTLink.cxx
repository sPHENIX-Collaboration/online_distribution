...
//{
//   std::string ss = fmt::format("link cruID:{:#06x}/lID{} feeID:{:#06x}",
//   cruID, int(idInCRU), feeID); if (lanes) {
...
//   LOG(info) << "Offs: " << offs << " Trigger : Orbit " << gbtTrg->orbit << "
//   BC: " << gbtTrg->bc << " Trigger: " << trb << " noData:"
//             << gbtTrg->noData << " internal:" << gbtTrg->internal << "
//             continuation:" << gbtTrg->continuation << " on " << describe();
//   gbtTrg->printX();
// }
//
/////_________________________________________________________________
...
//{
//   LOGP(info, "Offs: {} Calibration word {:5} | user_data {:#08x} on {}",
//   offs, gbtCal->calibCounter, gbtCal->calibUserField, describe());
//   gbtCal->printX();
// }
//
/////_________________________________________________________________
...
//   LOG(info) << "Offs: " << offs << " Header : Active Lanes " << LA << " on "
//   << describe(); gbtH->printX();
// }
//
/////_________________________________________________________________
...
////  LOG(info) << "Offs: " << offs << " HeaderL : Active Lanes " << LA << " on
///" << describe(); /  gbtH->printX(expectPadding);
////}
//
/////_________________________________________________________________
...
//   LOG(info) << "Offs: " << offs << " Trailer: Done=" << gbtT->packetDone << "
//   Lanes TO: " << LT << " | Lanes ST: " << LS << " on " << describe();
//   gbtT->printX();
// }
//
/////_________________________________________________________________
...
//{
//   LOGP(info, "Diagnostic for {} Lane {} | errorID: {} data {:#018x} on {}",
//   "IB", gbtD->getCableID(), gbtD->laneErrorID, gbtD->diagnosticData,
//   describe()); gbtD->printX();
// }
//
/////_________________________________________________________________
...
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrNoRDHAtStart]))
//     {
//       err |= uint8_t(ErrorPrinted);
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrNoRDHAtStart];
//     }
...
//   }
//   /*
//   if ((RDHUtils::getPacketCounter(rdh) > packetCounter + 1) && packetCounter
//   >= 0) {
//     if (irHBF.isDummy()) {
...
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrPacketCounterJump]))
//     {
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrPacketCounterJump]
//                 << " : jump from " << int(packetCounter) << " to " <<
...
////    gbtErrStatUpadated = true;
////    if
///(needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrWrongAlignmentWord]))
///{ /      err |= uint8_t(ErrorPrinted); /      LOG(info) << describe() << ' '
///<< statistics.ErrNames[GBTLinkDecodingStat::ErrWrongAlignmentWord] << " at
/// offset " << dataOffset << " for page size " << lastPageSize; /    } /
/// errorBits |= 0x1 << int(GBTLinkDecodingStat::ErrWrongAlignmentWord); / err
///|= uint8_t(Warning); /    return err; // fatal error /  }
//  return err;
//}
//
/////_________________________________________________________________
...
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrPageNotStopped]))
//     {
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrPageNotStopped];
...
//{
//   uint8_t err = uint8_t(NoError);
//   if (RDHUtils::getStopBit(rdh) && RDHUtils::getPacketSize(rdh) !=
//   sizeof(RDH) + sizeof(FLXWord)) { // there could be only 1 diagnostic
//   GBTWord after stop
//     statistics.errorCounts[GBTLinkDecodingStat::ErrStopPageNotEmpty]++;
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrStopPageNotEmpty]))
//     {
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrStopPageNotEmpty];
//       RDHUtils::printRDH(rdh);
//       err |= uint8_t(ErrorPrinted);
//     }
...
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrMissingGBTTrigger]))
//     {
//       gbtTrg->printX();
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrMissingGBTTrigger]; err |=
//       uint8_t(ErrorPrinted);
//     }
...
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrMissingGBTHeader]))
//     {
//       gbtH->printX();
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrMissingGBTHeader]; err |=
//       uint8_t(ErrorPrinted);
//     }
...
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrInvalidActiveLanes]))
//     {
//       std::bitset<32> expectL(cbl), gotL(lanesActive);
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrInvalidActiveLanes] << ' '
...
//{
//   uint8_t err = uint8_t(NoError);
//   lanesWithData |= 0x1 << cablePos;    // flag that the data was seen on this
//   lane if (lanesStop & (0x1 << cablePos)) { // make sure stopped lanes do not
//   transmit the data
//     statistics.errorCounts[GBTLinkDecodingStat::ErrDataForStoppedLane]++;
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrDataForStoppedLane]))
//     {
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrDataForStoppedLane] <<
//       cablePos; err |= uint8_t(ErrorPrinted);
//     }
...
//   gbtErrStatUpadated = true;
//   if
//   (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrGBTWordNotRecognized]))
//   {
...
//     gbtD->printX();
//     LOG(info) << describe() << ' ' << irHBF << ' ' <<
//     statistics.ErrNames[GBTLinkDecodingStat::ErrGBTWordNotRecognized]; err |=
//     uint8_t(ErrorPrinted);
//   }
//   err |= uint8_t(Skip);
//   return err;
...
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrMissingGBTTrailer]))
//     {
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrMissingGBTTrailer]; err |=
//       uint8_t(ErrorPrinted);
//     }
...
//{
//   uint8_t err = uint8_t(NoError);
//   if (!gbtT || (!gbtT->packetDone && notEnd)) { // Done may be missing only
//   in case of carry-over to new CRU page
//     statistics.errorCounts[GBTLinkDecodingStat::ErrPacketDoneMissing]++;
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrPacketDoneMissing]))
//     {
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrPacketDoneMissing]; err |=
//       uint8_t(ErrorPrinted);
//     }
...
//     if (RDHUtils::getTrgType(*lastRDH) != mvtx::TrgBitMap::SOT) { // only SOT
//     trigger allows unstopped lanes?
//       statistics.errorCounts[GBTLinkDecodingStat::ErrUnstoppedLanes]++;
//       gbtErrStatUpadated = true;
//       if
//       (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrUnstoppedLanes]))
//       {
//         std::bitset<32> active(lanesActive), stopped(lanesStop);
//         LOG(info) << describe() << ' ' << irHBF << ' ' <<
//         statistics.ErrNames[GBTLinkDecodingStat::ErrUnstoppedLanes]
...
//     err |= uint8_t(Warning);
//   }
//   // make sure all active lanes (except those in time-out) have sent some
//   data if ((~lanesWithData & lanesActive) != lanesTimeOut) {
//     statistics.errorCounts[GBTLinkDecodingStat::ErrNoDataForActiveLane]++;
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrNoDataForActiveLane]))
//     {
//       std::bitset<32> withData(lanesWithData), active(lanesActive),
//       timeOut(lanesTimeOut); LOG(info) << describe() << ' ' << irHBF << ' '
//       << statistics.ErrNames[GBTLinkDecodingStat::ErrNoDataForActiveLane]
//                 << " | with data: " << withData << " active: " << active << "
//                 timeOut: " << timeOut;
//       err |= uint8_t(ErrorPrinted);
//     }
...
//{
//   uint8_t err = uint8_t(NoError);
//   if (RDHUtils::getPacketSize(lastRDH) != sizeof(RDH) + sizeof(FLXWord) ||
//   !gbtD->isDiagnosticWord()) { //
//     statistics.errorCounts[GBTLinkDecodingStat::ErrMissingDiagnosticWord]++;
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrMissingDiagnosticWord]))
//     {
//       gbtD->printX();
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrMissingDiagnosticWord]; err
//       |= uint8_t(ErrorPrinted);
//     }
...
//     gbtErrStatUpadated = true;
//     if
//     (needToPrintError(statistics.errorCounts[GBTLinkDecodingStat::ErrWrongeCableID]))
//     {
//       gbtD->printX();
//       LOG(info) << describe() << ' ' << irHBF << ' ' <<
//       statistics.ErrNames[GBTLinkDecodingStat::ErrWrongeCableID] << ' ' <<
//       gbtD->getCableID(); err |= uint8_t(ErrorPrinted);
//     }
