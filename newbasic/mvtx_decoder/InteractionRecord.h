...
namespace mvtx
{
  namespace lhcConstants
  {
    ...
    /*
    constexpr double LHCRFFreq = 400.789e6;                          // LHC RF
    frequency in MHz constexpr double LHCBunchSpacingNS = 10 * 1.e9 / LHCRFFreq; //
    bunch spacing in ns (10 RFbuckets) constexpr double LHCOrbitNS = LHCMaxBunches *
    LHCBunchSpacingNS; // orbit duration in ns constexpr double LHCRevFreq = 1.e9 /
    LHCOrbitNS;                 // revolution frequency constexpr double
    LHCBunchSpacingMUS = LHCBunchSpacingNS * 1e-3;  // bunch spacing in \mus (10
    RFbuckets) constexpr double LHCOrbitMUS = LHCOrbitNS * 1e-3;                //
    orbit duration in \mus

    constexpr int RHICMaxBunches = 120;                              // max N
    bunches constexpr double RHICFFreq = 9.839e6;                            // RHIC
    RF frequency in MHz constexpr double RHICOrbitNS = 1.e9 / RHICFFreq; // RHIC BCO
    in nS
    */
  }  // namespace lhcConstants
  ...
  struct InteractionRecord
  {
    ...
    bool isDummy() const { return bc > mvtx::lhcConstants::LHCMaxBunches; }
