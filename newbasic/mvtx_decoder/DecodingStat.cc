...
constexpr std::array<std::string_view, ChipStat::NErrorsDefined>
    ChipStat::ErrNames;
#endif
  ...
  if (mask)
  {
    for (int i = NErrorsDefined; i--;)
    {
      if (mask & (0x1 << i))
      {
        ...
        {
          //          LOGP(important, "New error registered on the
          //          FEEID:{:#04x}: chip#{}: {}", feeID, chID, ErrNames[i]);
  ...
  if (d.getErrorFlags())
  {
    for (int i = NErrorsDefined; i--;)
    {
      if (d.getErrorFlags() & (0x1 << i))
      {
        ...
        if (verbosity > -1 && (!errorCounts[i] || verbosity > 1))
        {
          //          LOGP(info, "New error registered at bc/orbit {}/{} on the
          //          FEEID:{:#04x} chip#{}: {}{}",
          //               d.getInteractionRecord().bc,
          //               d.getInteractionRecord().orbit, feeID,
          //               int16_t(d.getChipID()), ErrNames[i],
          //               d.getErrorDetails(i));
          res |= ErrActions[i] & ErrActDump;
        }
        errorCounts[i]++;
...
void ChipStat::print(bool skipNoErr, const std::string &pref) const
{
  ...
  //  if (!skipNoErr || nErr) {
  //    std::string rep = fmt::format("{}#{:#04x} NHits: {}  errors: {}",
  //    pref.c_str(), feeID, nHits, nErr); for (int i = 0; i < NErrorsDefined;
  //    i++) {
  //      if (!skipNoErr || errorCounts[i]) {
  //        rep += fmt::format(" | Err.: {}: {}", ErrNames[i].data(),
  //        errorCounts[i]);
  //      }
  //    }
  //    LOG(info) << rep;
...
void GBTLinkDecodingStat::print(bool skipNoErr) const
{
  ...
  if (!skipNoErr || nErr)
  {
    //    std::string rep = fmt::format("FEEID#{:#04x} Packet States Statistics
    //    (total packets: {}, triggers: {})", feeID, nPackets, nTriggers); bool
    //    countsSeen = false; for (int i = 0; i <
    //    GBTDataTrailer::MaxStateCombinations; i++) {
    //      if (packetStates[i]) {
    //        if (!countsSeen) {
    //          countsSeen = true;
    ...
    //        rep += fmt::format("b{:s}: {}", patt.to_string().c_str(),
    //        packetStates[i]);
    //      }
    //    }
    ...
    //      if (!skipNoErr || errorCounts[i]) {
    //        rep += fmt::format(" [{}: {}]", ErrNames[i].data(),
    //        errorCounts[i]);
    //      }
    //    }
    //    LOG(info) << rep;
