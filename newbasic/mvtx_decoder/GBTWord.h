...
// @sa
// <O2/Detectors/ITSMFT/common/reconstruction/include/ITSMFTReconstruction/GBTWord.h>
//     <d03b22564>
...
namespace mvtx
{
  ...
  // GBT header flag for the ITS IB idagnostic : 101 bbbbb with bbbbb -> Lane
  // Number (0-8)
...
#pragma GCC diagnostic ignored "-Wpedantic"
    /// GBT word of 80 bits, bits 72:79 are reserver for GBT Header flag, the rest
    /// depends on specifications
    union
    {
      struct
      {
        uint64_t activeLanes : 28;  /// 0:27   Bit map of lanes active and eligible
                                    /// for readout
      ...
      // RS: packing will be needed only if some of the members cross 64 bit
      // boundary
      struct __attribute__((packed))
      {
        uint64_t triggerType : 12;  /// 0:11   12 lowest bits of trigger type
                                    /// received from CTP
        uint64_t internal : 1;      /// 12     Used in Continuous Mode for internally
                                    /// generated trigger
        uint64_t noData : 1;        /// 13     No data expected (too close to previous
                                    /// trigger or error)
        uint64_t continuation : 1;  /// 14     following data is continuation of
                                    /// the trigger from the previous CRU page
        ...
        //      uint8_t  id : 8;         /// = 0xe8; Trigger Data Header (TDH)
        //      identifier
      ...
      struct __attribute__((packed))
      {
        ...
        //    uint64_t id : 8;              /// 72:79  0xf8; Calibration Status
        //    Word (HSW) identifier
      };  /// Calibration Data Word

      struct
      {
        uint64_t lanesStatus : 56;          /// 0:55  Bit map of “Valid Lane stops
                                            /// received”, 1 bit per lane, NOT USED
        uint64_t na0t : 5;                  /// 56:60 reserved
        uint64_t timeout_in_idle : 1;       /// 61  = 1 if timeout waiting for a valid
                                            /// word from lanes
        uint64_t timeout_start_stop : 1;    /// 62  = 1 if timeout waiting for
                                            /// end-of-packet from all lanes
        uint64_t timeout_to_start : 1;      /// 63  = 1 if timeout waiting for first
                                            /// word from first lane
        uint64_t packet_done : 1;           /// 64  = 1 when current trigger packets
                                            /// transmission done
        uint64_t transmission_timeout : 1;  /// 65  = 1 if timeout while waiting
                                            /// for data on lanes
        uint64_t na1t : 1;                  /// 66  reserved
        uint64_t
            lane_starts_violation : 1;  /// 67  = 1 if at least 1 lane (eligible
                                        /// for readout) had a “start violation”
        uint64_t lane_timeouts : 1;     /// 68  = 1 if at least 1 lane (eligible for
                                        /// readout) timed out
        uint64_t na2t : 3;              /// 69:71  reserved
        //        uint8_t  id : 8;                 /// = 0xf0; Trigger Data
        //        Trailer (TDT) identifier
      };  // TRIGGER DATA TRAILER

      struct
      {
        uint64_t lane_status : 56;            /// 0:55  Readout status of the lanes for the
                                              /// current HBF.
                                              ///       Higher status level achieved for each
                                              ///       lane taken from each TDT.
        uint64_t na3tr : 8;                   /// 56:63 reserved
        uint64_t na4tr : 1;                   /// 64    reserved
        uint64_t transmission_timeouts : 1;   /// 65    = 1 One or more lanes had a
                                              /// timeout in this HBF.
                                              ///       Cummulative logic OR of
                                              ///       transmission_timeout field
                                              ///       of the TDTs.
        uint64_t na5tr : 1;                   /// 66    reserved
        uint64_t lane_starts_violations : 1;  /// 67    = 1 One or more lanes had a
                                              /// protocol violation in this HBF.
                                              ///       Cummulative logic OR of
                                              ///       transmission_timeout field
                                              ///       of the TDTs.
        uint64_t index : 4;                   /// 68:71 Must be = 0 DDW0
        //      uint64_t id : 8;              /// 72:79  0xe4;  Status Word (HSW)
        //      identifier
      ...
      struct
      {
        ...
        uint8_t lane_error_id : 8;      /// 64:71 Identifier of the specific error
                                        /// condition
        //      uint8_t id : 8;               /// 72:79 0xa0 - 0xa8; Diagnostic IB
        //      lane
      };  // DIAGNOSTIC IB LANE
...
#pragma GCC diagnostic pop
    ...
    /// check if the GBT Header corresponds to ITS IB diagnostics data (header is
    /// combined with lanes info)
    bool isDiagnosticIB() const { return (id & 0xe0) == GBTFlagDiagnosticIB; }

    /// check if the GBT Header corresponds to ITS IB data (header is combined
    /// with lanes info)
