...
// @sa
// <O2/Detectors/ITSMFT/common/reconstruction/include/ITSMFTReconstruction/DecodingStat.h>
//     <5ca140c87>
...
namespace mvtx
{
  ...
  struct ChipStat
  {
    ...
    enum DecErrors : int
    {
      ...
      APE_STRIP,                     // lane data stripped for this chip event (behaviour changed with
                                     // RU FW v1.16.0, for general APE behaviour see
                                     // https://alice.its.cern.ch/jira/browse/O2-1717)
      ...
      APE_OCCUPANCY_RATE_LIMIT_2,    // pending detector events limit in
                                     // packager(FATAL)
    ...
    const std::array<const std::string, NErrorsDefined> ErrNames = {
        ...
        "APE_STRIP",                                     // lane data stripped for this chip event (behaviour changed
                                                         // with RU FW v1.16.0, for general APE behaviour see
                                                         // https://alice.its.cern.ch/jira/browse/O2-1717)
        "APE_RESERVED_F3",                               // reserved F3
        "APE_DET_TIMEOUT",                               // detector timeout (FATAL)
        "APE_OOT_START",                                 // 8b10b OOT (FATAL, start)
        ...
        "Chip data interleaved on the cable",            // Chip data interleaved on the
                                                         // cable
        "TruncatedBuffer",                               // truncated buffer, 0 padding
        "TrailerAfterHeader"                             // trailer seen after header w/o FE of FD set
    };
    ...
    static constexpr std::array<uint32_t, NErrorsDefined> ErrActions = {
        ...
        ErrActPropagate |
            ErrActDump,                // Region is not followed by Short or Long data
        ...
        ErrActPropagate |
            ErrActDump,                // lane data stripped for this chip event (behaviour
                                       // changed with RU FW v1.16.0, for general APE behaviour
                                       // see  https://alice.its.cern.ch/jira/browse/O2-1717)
        ...
        ErrActPropagate |
            ErrActDump,                // event protocol error marker (FATAL, start)
        ErrActPropagate | ErrActDump,  // lane FIFO overflow error (FATAL)
        ErrActPropagate |
            ErrActDump,                // FSM error (FATAL, SEU error, reached an unknown state)
        ErrActPropagate | ErrActDump,  // pending detector events limit (FATAL)
        ErrActPropagate |
            ErrActDump,                // pending detector events limit in packager(FATAL)
        ...
        ErrActPropagate |
            ErrActDump,               // Truncated buffer while something was expected
  ...
  struct GBTLinkDecodingStat
  {
    ...
    enum DecErrors : int
    {
      ...
      ErrPageNotStopped,            // RDH is stopped, but the time is not matching the ~stop
                                    // packet
      ErrStopPageNotEmpty,          // Page with RDH.stop is not empty
      ErrPageCounterDiscontinuity,  // RDH page counters for the same RU/trigger
                                    // are not continuous
      ...
      ErrNonZeroPageAfterStop,      // all lanes were stopped but the page counter in
                                    // not 0
      ErrUnstoppedLanes,            // end of FEE data reached while not all lanes received
                                    // stop
      ErrDataForStoppedLane,        // data was received for stopped lane
      ErrNoDataForActiveLane,       // no data was seen for lane (which was not in
                                    // timeout)
      ErrIBChipLaneMismatch,        // chipID (on module) was different from the lane ID
                                    // on the IB stave
      ErrCableDataHeadWrong,        // cable data does not start with chip header or
                                    // empty chip
      ErrInvalidActiveLanes,        // active lanes pattern conflicts with expected for
                                    // given RU type
      ErrPacketCounterJump,         // jump in RDH.packetCounter
      ErrPacketDoneMissing,         // packet done is missing in the trailer while CRU
                                    // page is not over
...
#endif
        "ORBIT", "HB", "HBr", "HC", "PHYSICS", "PP", "CAL", "SOT",
        "EOT", "SOC", "EOC", "TF", "FE_RST", "RT", "RS"};
