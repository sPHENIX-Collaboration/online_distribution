...
oncsSub_idmvtxv2::oncsSub_idmvtxv2(subevtdata_ptr data)
...
int oncsSub_idmvtxv2::encode_hit(unsigned short row, unsigned short col) const
{
    return (row << 16) + col;
}
...
bool oncsSub_idmvtxv2::mask_contains_ruchn ( int mask, int ruchn )
{
...
int *oncsSub_idmvtxv2::decode ()
{
    ...
    vector<unsigned char> ruchn_stream[IDMVTXV2_MAXRUID+1][IDMVTXV2_MAXRUCHN+1];

    unsigned char felix_counter [IDMVTXV2_MAXRUID+1];

    for( int i=0; i < IDMVTXV2_MAXRUID+1; i++)
    {
        felix_counter[i] = 0;
    }
    ...
    /*
       if (ruchn_stream[0].size()!=9) for ( int ruchn = 0; ruchn < IDMVTXV2_MAXRUCHN+1; ruchn++)
       {
       cout << __FILE__ << " " << __LINE__ << " --- ruchn " << ruchn << " has " << ruchn_stream[ruchn].size() << " bytes" << endl;
       }
       */

    for ( int ruid = 1; ruid < IDMVTXV2_MAXRUID+1; ruid++)
    {
        for ( int ruchn = 1; ruchn < IDMVTXV2_MAXRUCHN+1; ruchn++)
        {
...
int oncsSub_idmvtxv2::iValue(const int ruid)
{
    decode();
...
int oncsSub_idmvtxv2::iValue(const int ruid, const int ruchn, const char *what)
{
    decode();
...
int oncsSub_idmvtxv2::iValue(const int ruid, const int ruchn)
{
    decode();
...
int oncsSub_idmvtxv2::iValue(const int ruid, const int ruchn, const int i)
{
    decode();
...
void  oncsSub_idmvtxv2::dump ( OSTREAM& os )
{
...
void oncsSub_idmvtxv2::gdump(const int i, OSTREAM& out) const
{
